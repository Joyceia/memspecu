...
class Metrics:
    ...
    @staticmethod
    def get_topk_action_accuracy(normal_dict, sim_dict, k=1, skip_first=False):
        normal_actions = normal_dict.get("actions", [])
        sim_actions = sim_dict.get("actions", [])
        n = min(len(normal_actions), len(sim_actions))
        if n == 0:
            return None
        ...
        for idx in range(start_idx, n):
            ...
            if any(Metrics.compare_action(na, cand, sparse=False) for cand in candidates):
                hits += 1
            total += 1
    ...
    @staticmethod
    def get_action_specific_avg_metric_top1_top3(dir_path, get_time=False):
        metric_names = ["general", "Search", "Lookup", "Finish"]
        avg_metrics_dict = {
            "normal_avg_searchtime": 0,
            "sim_avg_searchtime": 0,
        }
        for metric in metric_names:
            avg_metrics_dict[f"{metric}_top1"] = 0
            avg_metrics_dict[f"{metric}_top3"] = 0
        ...
        for direc in os.listdir(dir_path):
            try:
                ...
                metric_dict_top1 = Metrics.get_action_metrics(
                    normal_obs_dict, sim_obs_dict, k=1, sparse=False
                )
                metric_dict_top3 = Metrics.get_action_metrics(
                    normal_obs_dict, sim_obs_dict, k=3, sparse=False
                )

                for metric in metric_names:
                    avg_metrics_dict[f"{metric}_top1"] += metric_dict_top1.get(metric, 0)
                    avg_metrics_dict[f"{metric}_top3"] += metric_dict_top3.get(metric, 0)
        ...
        for metric in metric_names:
            top1_key = f"{metric}_top1"
            top3_key = f"{metric}_top3"
            try:
                avg_metrics_dict[top1_key] /= n_metrics
                avg_metrics_dict[top3_key] /= n_metrics
            except ZeroDivisionError:
                avg_metrics_dict[top1_key] = None
                avg_metrics_dict[top3_key] = None
