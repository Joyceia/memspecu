...
│   ├── llm_client.py           # Multi-provider LLM client (OpenRouter, Gemini, OpenAI, vLLM)
...
├── start_vllm_server.py        # vLLM server launcher script
├── VLLM_SETUP.md               # vLLM setup and configuration guide
...
Open `src/constants.py` and configure your preferred LLM provider:

#### Option A: Use API providers (OpenAI, Gemini, OpenRouter)
...
#### Option B: Use local open-source models with vLLM

**Recommended approach for cost-effective inference.**

First, set up vLLM (see [VLLM_SETUP.md](VLLM_SETUP.md) for detailed instructions):

```bash
# Start vLLM server with a local model
...
Or configure in `src/constants.py`:
...
Direct Gemini and OpenAI clients are also supported — add the relevant API keys to `constants.py` to use them.
